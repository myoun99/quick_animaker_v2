...
void main() {
  group('Phase 223 pixel-grid brush display architecture', () {
    test('production active brush view uses raster surfaces instead of path painter display', () {
      ...
      expect(canvasView, isNot(contains('displayPreviewSurface')));
      expect(interactive, isNot(contains('drawPath')));
      expect(interactive, isNot(contains('Path()')));
    });

    test('active brush panel does not pass inactive preview cache as active display path', () {
      final panel = File('lib/src/ui/brush/brush_canvas_panel.dart').readAsStringSync();
    ...
    test('brush runtime architecture does not reintroduce TileDelta command usage', () {
      ...
      for (final file in runtimeFiles) {
        ...
        expect(source, isNot(contains('TileDeltaCommand')), reason: file.path);
      }
    });
  });
}
