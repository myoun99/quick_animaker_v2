...
void main() {
  group('TileDeltaCommand', () {
    ...
    test('deltas getter is unmodifiable', () {
      ...
      expect(() => command.deltas.add(TileDelta.created(tile(1, 0))), throwsUnsupportedError);
    });
    ...
    test('dirtyTiles returns DirtyTileSet of delta coords', () {
      final command = TileDeltaCommand(deltas: [
        TileDelta.created(tile(0, 0)),
    ...
    test('length returns delta count', () {
      expect(TileDeltaCommand(deltas: [TileDelta.created(tile(0, 0))]).length, 1);
    });

    test('containsCoord returns true for existing coord', () {
      expect(TileDeltaCommand(deltas: [TileDelta.created(tile(0, 0))]).containsCoord(TileCoord(x: 0, y: 0)), isTrue);
    });

    test('containsCoord returns false for missing coord', () {
      expect(TileDeltaCommand(deltas: [TileDelta.created(tile(0, 0))]).containsCoord(TileCoord(x: 1, y: 0)), isFalse);
    });

    test('deltaFor returns delta for coord', () {
      final delta = TileDelta.created(tile(0, 0));
      expect(TileDeltaCommand(deltas: [delta]).deltaFor(TileCoord(x: 0, y: 0)), delta);
    });

    test('deltaFor returns null for missing coord', () {
      expect(TileDeltaCommand(deltas: [TileDelta.created(tile(0, 0))]).deltaFor(TileCoord(x: 1, y: 0)), isNull);
    });

    test('equality ignores insertion order', () {
      final a = TileDelta.created(tile(0, 0));
      final b = TileDelta.created(tile(1, 0));
      expect(TileDeltaCommand(deltas: [a, b]), TileDeltaCommand(deltas: [b, a]));
    });

    test('hashCode ignores insertion order', () {
      final a = TileDelta.created(tile(0, 0));
      final b = TileDelta.created(tile(1, 0));
      expect(TileDeltaCommand(deltas: [a, b]).hashCode, TileDeltaCommand(deltas: [b, a]).hashCode);
    });
    ...
    test('toJson emits deterministic delta order', () {
      ...
      final coords = (command.toJson()['deltas'] as List)
          ...
          .toList();
      expect(coords, [{'x': 1, 'y': 0}, {'x': 0, 'y': 1}, {'x': 2, 'y': 1}]);
    });

    test('validateAgainstSurface accepts in-bounds matching tileSize deltas', () {
      expect(() => TileDeltaCommand(deltas: [TileDelta.created(tile(2, 2))]).validateAgainstSurface(surface()), returnsNormally);
    });

    test('validateAgainstSurface rejects coord outside surface', () {
      expect(() => TileDeltaCommand(deltas: [TileDelta.created(tile(3, 0))]).validateAgainstSurface(surface()), throwsArgumentError);
    });

    test('validateAgainstSurface rejects tile size mismatch', () {
      expect(() => TileDeltaCommand(deltas: [TileDelta.created(tile(0, 0, size: 3))]).validateAgainstSurface(surface()), throwsArgumentError);
    });

    test('applyAfter creates missing tile', () {
      final after = tile(0, 0, firstByte: 1);
      expect(TileDeltaCommand(deltas: [TileDelta.created(after)]).applyAfter(surface()).tileAt(after.coord), after);
    });

    test('applyBefore removes created tile', () {
      final after = tile(0, 0, firstByte: 1);
      final result = TileDeltaCommand(deltas: [TileDelta.created(after)]).applyBefore(surface().putTile(after));
      expect(result.tileAt(after.coord), isNull);
    });

    test('applyAfter removes deleted tile', () {
      final before = tile(0, 0, firstByte: 1);
      final result = TileDeltaCommand(deltas: [TileDelta.removed(before)]).applyAfter(surface().putTile(before));
      expect(result.tileAt(before.coord), isNull);
    });

    test('applyBefore restores deleted tile', () {
      final before = tile(0, 0, firstByte: 1);
      final result = TileDeltaCommand(deltas: [TileDelta.removed(before)]).applyBefore(surface());
    ...
    test('applyAfter replaces modified tile', () {
      ...
      final after = tile(0, 0, firstByte: 1);
      final result = TileDeltaCommand(deltas: [TileDelta.replaced(before: before, after: after)]).applyAfter(surface().putTile(before));
    ...
    test('applyBefore restores original modified tile', () {
      final before = tile(0, 0);
      final after = tile(0, 0, firstByte: 1);
      final result = TileDeltaCommand(deltas: [TileDelta.replaced(before: before, after: after)]).applyBefore(surface().putTile(after));
      expect(result.tileAt(before.coord), before);
    });
