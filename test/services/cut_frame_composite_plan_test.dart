...
void main() {
  ...
  group('visibility solo override (R3 #3)', () {
    Layer plain(String id, int marker, {bool isVisible = true}) => Layer(
      id: LayerId(id),
      name: id,
      frames: [frame('frame-$marker')],
      isVisible: isVisible,
      timeline: {
        0: TimelineExposure.drawing(FrameId('frame-$marker'), length: 1),
      },
    );

    test('a non-null soloVisibleLayerId REPLACES the eye predicate: only '
        'the soloed layer contributes, its own eye ignored', () {
      final layers = [
        plain('a', 1),
        plain('b', 2, isVisible: false),
        plain('c', 3),
      ];

      final soloB = planCutFrameComposite(
        cut: cut(layers),
        frameIndex: 0,
        surfaceResolver: resolver,
        soloVisibleLayerId: const LayerId('b'),
      );
      // b displays even though its eye is off (solo overrides eyes); the
      // visible a/c rows are hidden by the solo.
      expect(soloB.map(markerOf), [2]);

      // No solo: normal eye semantics.
      final normal = planCutFrameComposite(
        cut: cut(layers),
        frameIndex: 0,
        surfaceResolver: resolver,
      );
      expect(normal.map(markerOf), [1, 3]);
    });

    test('soloing a base carries its attach rows (part of the group)', () {
      final base = Layer(
        id: const LayerId('base'),
        name: 'A',
        frames: [frame('frame-1')],
      ...
      final attach = Layer(
        id: const LayerId('plus'),
        name: 'A +',
        frames: [frame('frame-2')],
        timeline: const {},
        attachedToLayerId: const LayerId('base'),
        baseFrameLinks: {const FrameId('frame-1'): const FrameId('frame-2')},
      );
      final other = plain('other', 3);

      final solo = planCutFrameComposite(
        cut: cut([base, attach, other]),
        frameIndex: 0,
        surfaceResolver: resolver,
        soloVisibleLayerId: const LayerId('base'),
      );
      expect(solo.map(markerOf), [1, 2]);
    });
  });
}
