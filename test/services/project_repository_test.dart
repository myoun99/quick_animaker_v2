...
void main() {
  group('ProjectRepository', () {
    ...
    test('adds a cut to an existing track', () {
      final track = _track(id: 'track-1', name: 'Video');
      final project = _project(id: 'project-1', name: 'Project', tracks: [track]);
    ...
    test('adds a layer to an existing cut', () {
      ...
      expect(repository.requireProject().tracks.single.cuts.single.layers, [layer]);
    ...
    test('adds a frame to an existing layer', () {
      ...
      expect(
        repository.requireProject().tracks.single.cuts.single.layers.single.frames,
        [frame],
      );
    ...
    test('adds a stroke to an existing frame', () {
      ...
      expect(project.tracks.single.cuts.single.layers.single.frames.single.strokes, isEmpty);
    });
    ...
    test('keeps original project and frame unchanged after adding a stroke', () {
      ...
      final updatedTrack = updatedProject.tracks.single;
      final updatedCut = updatedTrack.cuts.single;
      ...
      expect(updatedTrack, isNot(same(originalTrack)));
      expect(updatedCut, isNot(same(originalCut)));
      expect(updatedLayer, isNot(same(originalLayer)));
      expect(updatedFrame, isNot(same(originalFrame)));
      expect(updatedFrame.strokes, [stroke]);
      expect(
        originalProject.tracks.single.cuts.single.layers.single.frames.single.strokes,
        isEmpty,
      );
