...
class CanvasViewportPanMetrics {
  ...
  CanvasViewportPanMetrics({
    ...
    required double trackExtent,
       ...
       scaledContentExtent = _scaledContentExtent(axis, viewport, canvasSize),
    ...
    final scroll = (-pan).clamp(0.0, maxScroll).toDouble();
  ...
  final double scaledContentExtent;
  ...
  CanvasViewport panToThumb(double thumbStart) {
    ...
    return axis == Axis.horizontal
        ? viewport.copyWith(panX: -scroll)
        : viewport.copyWith(panY: -scroll);
  }
  ...
  static double _scaledContentExtent(
    Axis axis,
    ...
    final source = axis == Axis.horizontal
        ? canvasSize.width
        : canvasSize.height;
    return _finiteNonNegative(source * viewport.zoom);
  }
