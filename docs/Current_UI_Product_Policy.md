...
Every scrollbar in the app is the shared `AppScrollbar` (`lib/src/ui/widgets/app_scrollbar.dart`): a thin grey thumb (4px) that brightens on hover and turns accent while dragged, with no track, centered in a 12-16px hit lane. `ScrollController`-backed surfaces use `AppControllerScrollbar`; panel scrollables wrap in `PanelScrollbar` (`lib/src/ui/panels/panel_scrollbar.dart`) and pad the scroll-end edge by `panelScrollbarGutter`; the canvas panbars and timeline rails delegate to the same widget from their existing lanes. Do not restyle scrollbars per surface — the app-level `ScrollbarThemeData` mirrors the same visuals for any stray raw `Scrollbar`.
...
- UNIFIED TIMELINE MODEL: `Layer.timeline` is the single frame-axis record. An entry is either a drawing block start (`frameId` + explicit hold `length`; the block covers `[start, start+length)`) or an inbetween mark. Emptiness is NOT data — uncovered cells render the timesheet "X" glyph (every empty cel cell inside the playback range; camera rows excepted). There is no blank entry type and no separate marks map (legacy files migrate on load: blanks become the preceding block's cut point, marks merge into the timeline, length-less drawings take their old visual length). Marks live on held or empty cells only — never on a drawing start — never form blocks, and never affect canvas/playback resolution (a covered cell shows its block's frame; an uncovered cell shows nothing). The old infinite trailing hold is gone: every block has a true end. The "Blank / X" toolbar action now CUTS the covering block's hold at the current cell. Hold length lives on the timeline entry, so linked uses of one frame can hold for different lengths (`Frame.duration` is no longer authoritative). Coverage queries are shared via `timeline_coverage.dart` (SplayTreeMap navigation, O(log n)) so the controller, playback compositing, and UI always agree.
- Comma-drag exposure editing is TVPaint-style and shared across orientations (`timeline_exposure_comma_drag_policy.dart` / `timeline_exposure_comma_drag_handle.dart`, step math orientation-free; LayerKind dispatch inside shared row widgets — camera rows get no grips). EVERY drawing block shows two inset edge grips (vertical bars just inside its start/end edges). Dragging an edge is a mirror ripple: the glued chain on that side moves rigidly with the edge (commas preserved), separated blocks absorb the empty "X" gap first and move only on contact, and X area consumed or opened stays rendered as X. With nothing connected in front, a start-edge drag just changes the block's own comma. Drags preview live from a drag-start snapshot with the cumulative frame delta (idempotent) and commit as ONE undo entry on release; the toolbar +/- buttons are the same edge op with delta ±1 (so the last block can now shrink too).
