...
## 중요 : 인수인계문 수정할 시, 0번부터 4번까지는 사용자가 수정한다. AI는 6번부터만 수정을 허용한다.
...
## 5. Current documentation entry point

This handoff is now an entry/rules/index document. Detailed module architecture must live in the `Current_*` source-of-truth documents, not in this handoff.
...
- Always read sections 0 through 4 of this handoff first; they are user-owned.
- For architecture details, read the relevant `Current_*` document before changing code or writing a new phase task.
- Use `Current_` for current docs. Do not use removed phase tasks, obsolete decision notes, checkpoint notes, review notes, memo notes, or prior long-term drafts as active policy.
- If a detailed module rule is missing, update the appropriate `Current_*` document rather than expanding this handoff into detailed architecture.
