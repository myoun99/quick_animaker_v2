...
A preset is a named `BrushSettings` snapshot (`BrushPreset` model) with an optional `group` (the import source file's base name — Clip-Studio-like sub-tool groups; `null` = ungrouped, shown under "Default"). Import assigns the group at the `EditorCanvasArea` merge site (decoders stay group-agnostic); the panel renders collapsible group headers in first-appearance order and omits headers entirely when every preset is ungrouped. The library is app-level editor state, NOT project data: it persists in a per-user JSON file (`%APPDATA%/quick_animaker_v2/brush_presets.json` via `BrushPresetFileService`, no extra dependencies) and never enters the project save schema. A missing or corrupt library file falls back to the built-in defaults (`defaultBrushPresets`: Pencil, Ink Pen, Soft Brush, Calligraphy, Marker — seeded only in memory; the file is written on the first user save). An explicitly saved empty library stays empty.

`EditorCanvasArea` owns the loaded preset list alongside `BrushToolState`, converts through `BrushToolState.fromBrushSettings` / `toBrushSettings` (clamping into panel ranges on apply), and persists mutations fire-and-forget (a failed write must never block or crash the editor). The library UI is the dedicated `BrushPresetPanel` ("Brushes") stacked above `BrushSettingsPanel` in the right dock (Clip-Studio-like list/properties split): one row per preset showing a `BrushStrokePreview` — a synchronous S-curve stroke sample rasterized through the shared `brushPixelCoveragesForDab` oracle (so sampled tips, roundness/angle, hardness, dual masks, and paper texture render honestly; size is normalized to the row height, a synthetic 0-1-0 pressure arc tapers the stroke when the pressure toggles are on, and placement dynamics are skipped so the preview stays deterministic; the buffer is cached per settings+size). Each row shows a `BrushTipPreview` icon (coarse mask-average grid for sampled tips, parametric shape otherwise), the stroke preview, and the name; the options menu carries per-element view toggles (icon/stroke/name — the last visible element cannot be hidden, and the view state is panel-local editor UI, not persisted). Tap applies; destructive actions live behind the same header options menu (Photoshop-style `brush-preset-menu-button`, acting on the selected preset — no per-row delete affordance, so a stray click cannot delete a brush). The header also hosts import and save-as-preset. `EditorCanvasArea` tracks the last-applied/saved preset id and highlights that row; tweaking settings keeps the highlight, deleting the preset clears it. Applying a preset only changes future strokes (the usual pointer-down snapshot rule).
...
Built-in sampled tips (`brush_tip_mask_defaults.dart`: Chalk grain disc, Splatter droplets) are generated with a fixed-seed LCG so the same bytes are produced on every run — fingerprint-locked by test, because changing them would re-render existing strokes. They ship as the Chalk/Splatter presets; the preset library file version bumped to 2, and older library files gain newly added built-ins once on load (deletions within the current version are respected). There is no direct mask picker in the panel yet — masks arrive via presets and ABR import.

## ABR import (P18)

`decodeAbrBrushFile` (`services/abr/`) parses Photoshop `.abr` files, versions 6/7/10 (the format every modern Photoshop writes; v1/v2 rejected with a clear error). The `samp` 8BIM section provides sampled tip bitmaps (8-bit grayscale, raw or PackBits RLE with per-scanline lengths; 16-bit tips are skipped with a warning; non-square tips are padded to a centered square). The `desc` section is a serialized Photoshop ActionDescriptor, parsed by the minimal recursive parser in `photoshop_descriptor.dart` (Objc/VlLs/UntF/doub/long/bool/TEXT/enum/tdta/...). From each brush entry we map: name, `Dmtr` -> size, `Spcn`% -> spacing ratio, `Angl` -> angleDegrees (normalized into 0-180), `Rndn`% -> roundness, `Hrdn`% -> hardness (computed brushes), `sampledData` UUID -> tip mask join. Dynamics (size/opacity jitter, scattering, texture, dual brush, wet edges) are NOT mapped yet.

Robustness rules: a corrupt or unparseable `desc` degrades to importing the tip bitmaps with default settings plus a warning — tip bitmaps are never silently dropped; unreferenced `samp` tips import as "<file> tip N". Preset ids are `abr-<tip uuid>` (or `abr-<file>-computed-N`), so re-importing the same file REPLACES those presets instead of duplicating them. Import is wired through the panel's import button (`brush-preset-import-button`) -> `EditorCanvasArea._importBrushFile` -> platform file dialog (`file_selector`, the project's first non-SDK dependency) -> merged into the preset library and persisted; results surface in a SnackBar.
...
The interactive canvas view reads each pointer sample's normalized pressure — `(pressure - pressureMin) / (pressureMax - pressureMin)`, clamped to 0..1. Only stylus device kinds (`stylus`/`invertedStylus`) are trusted for pressure: a mouse claims a 0..1 pressure range on some platforms while always reporting 0.0 (which made pressure-sized strokes invisible), and touch pressure is unreliable across devices — both paint at full pressure, so enabling the toggles never changes mouse behavior. `_dabFromPosition` builds each dab with the base tool size/opacity and that raw pressure; `BrushDabInterpolator` interpolates the `pressure` field linearly across inserted dabs; then `applyBrushPressureDynamics` (services) scales `size`/`opacity` per dab to `base * pressure` for whichever toggle is on. Scaling is applied AFTER interpolation so each inserted dab scales by its own interpolated pressure rather than the segment endpoint's — a pre-interpolation scale would step the size across a segment. Spacing uses the base (un-scaled) size, so dab density stays consistent regardless of pressure and low-pressure strokes never gap.
...
activeFrameDisplay =
  ...
  + live stroke overlay tile images (exact incremental CPU rasterization)
```
...
The in-progress stroke is rasterized incrementally by `BrushLiveStrokeRasterizer`: each pointer move blends only the new dabs into a canvas-sized straight-alpha buffer using the exact per-dab math of the commit rasterizer (byte-identical, locked by `active_stroke_overlay_parity_test.dart`). The `ActiveStrokeOverlayModel` snapshots the touched overlay tiles (128px grid, independent of the surface tile size) from that buffer, premultiplies them, and re-decodes them with `decodeImageFromPixels` — the EXACT pipeline the committed tile images use. `BitmapSurfacePainter` draws the overlay tile images with the same nearest-sampling paint as the committed tiles (overlay tiles never overlap, so plain source-over per tile is exact). Decode completions repaint through the `ActiveStrokeOverlayModel` ChangeNotifier without rebuilding widgets; a tile touched while its decode is in flight re-snapshots the newer buffer state as soon as the running decode lands, bounding per-frame work to one decode per touched tile.

One rasterization path for live and committed pixels is the load-bearing property: rendering the live stroke as rect geometry (`ui.Picture` run rects, the P11 representation) rasterized differently from nearest-sampled images at fractional zoom and visibly shifted the active stroke's pixels against committed strokes (100%/high zoom fine, ~125% severe). Decode-based images also survive GPU context events (app focus switches) that corrupted synchronously created `toImageSync` textures for a frame at stroke start/end. A guard in `brush_tile_delta_eradication_test.dart` bans synchronous GPU-image conversion from the overlay display path.
...
- Never use replacement blend modes (`BlendMode.src`) on the transformed view canvas; region replacement composes only off-screen in image space, and finished content reaches the view canvas with plain source-over.
...
- Live and committed pixels must render through ONE rasterization path — nearest-sampled decoded images for both. Rect geometry (or picture replay) for the live stroke rasterizes differently from image sampling at fractional zoom and shifts the active stroke's pixels.
- Overlay images come from `decodeImageFromPixels` like the committed tiles, never from synchronous picture-to-GPU-image conversion: context-backed textures can flash garbage for a frame when created/disposed or when the GPU context is lost.
- Never dispose a display image in step with its replacement: the frame on screen can still reference the old texture while the raster thread works, and destroying it mid-flight intermittently flashed the tile as an opaque black square for one frame (GC-run tile-image finalizers at pen-up were the main trigger). All display-image disposal routes through `DeferredImageDisposer.retire`, which holds images for a few frame boundaries.
...
- Committed strokes display from the materialized session `BitmapSurface`; the in-progress stroke displays from its exact incremental rasterization (`BrushLiveStrokeRasterizer` -> decoded overlay tile images).
