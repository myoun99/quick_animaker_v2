...
`EditorCanvasArea` owns the loaded preset list alongside `BrushToolState`, converts through `BrushToolState.fromBrushSettings` / `toBrushSettings` (clamping into panel ranges on apply), and persists mutations fire-and-forget (a failed write must never block or crash the editor). The library UI is the dedicated `BrushPresetPanel` ("Brushes") stacked above `BrushSettingsPanel` in the right dock (Clip-Studio-like list/properties split): one row per preset showing a `BrushStrokePreview` — a synchronous S-curve stroke sample rasterized through the shared `brushPixelCoveragesForDab` oracle (so sampled tips, roundness/angle, hardness, dual masks, and paper texture render honestly; size is normalized to the row height, a synthetic 0-1-0 pressure arc tapers the stroke when the pressure toggles are on, and placement dynamics are skipped so the preview stays deterministic; the buffer is cached per settings+size). Tap applies; destructive actions live behind the header options menu (Photoshop-style `brush-preset-menu-button`, acting on the selected preset — no per-row delete affordance, so a stray click cannot delete a brush). The header also hosts import and save-as-preset. `EditorCanvasArea` tracks the last-applied/saved preset id and highlights that row; tweaking settings keeps the highlight, deleting the preset clears it. Applying a preset only changes future strokes (the usual pointer-down snapshot rule).
